...
var IMG_AJAXPATH = "/imgUpload";//异步传输服务端位置
...
$("body").delegate(".imgfile","change",function(){  
...
//删除选中图片
$("body").delegate(".lookimg_delBtn","click",function(){
// $(".lookimg_delBtn").on("click", function () {
    $(".imgfile[num=" + $(this).parent().attr("num") + "]").remove();//移除图片file
    $(this).parent().remove();//移除图片显示
});

//删除按钮移入移出效果
$("body").delegate(".lookimg","mouseover",function(){  
// $(".lookimg").on("mouseover", function () {
    if ($(this).attr("ISUP") != "1")
        $(this).children(".lookimg_delBtn").eq(0).css("display", "block");;
});
$("body").delegate(".lookimg","mouseout",function(){
// $(".lookimg").on("mouseout", function () {
...
$("#btn_ImgUpStart").click(function () {
    ...
    for (var i = 0; i < $(".lookimg").length; i++) {
        ...
        IMG_BASE = IMG_BASE.replace(/^data:image\/\w+;base64,/, "");
        ...
        if (IMG_ENDFOUR.trim() == ".jpg")
            ...
            IMG_FOMATE = "png";
        var resultstring = '{ "imgBase": "'+IMG_BASE+'", "imgFormat": "'+IMG_FOMATE+'", "lookIndex": '+NOWLOOK.index +', "imgName": "'+currentImg+'"}';
